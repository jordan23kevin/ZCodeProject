...
- **遮罩生成（胚衣制作页 `/peiyi`）** — 为模特胚衣图自动生成「衣身遮罩」+「遮挡物遮罩（手/戒指/头发/手持物）」，贴图时把印花藏到前景后面，避免「印花盖在手上」
- **遮罩版本归档与评分** — 每次生成存 `vNNN` 版本（`latest.txt` 指向生产）；顶部「📊 遮罩评分总表」低分排前标红；支持切换/删除版本
- **遮罩手动校正** — 点「② 分层预览」图做 LAB 色度连通域扩散（加遮挡/加衣身/减遮挡）；预览→确认才存版本，杜绝废版
- **导入手动 PS 遮罩合并** — 读取用户 PS 遮罩 PNG（alpha>30 即画笔），与 AI 遮罩合并；用户画笔触碰的 AI 连通域整块替换为用户精确轮廓，衣身不外扩
