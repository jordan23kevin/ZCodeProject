...
脚本目前仅在出错路径调用 `browser.close()`，成功完成时浏览器保持打开。将出错路径的关闭也改为受 `--no-close-browser` 控制：
...
```javascript
if (!drawerOk) {
  console.log('错误: 抽屉未打开');
  if (!keepOpen) await browser.close();
  return;
}
```

Bridge 启动时传 `--no-close-browser`，避免早期出错时把用户的 Edge 关掉；单独命令行运行时不传，行为保持原样。
...
6. 单独运行 `cd "E:/Claude code/WB Lovart" && node 建议零售价.js` 行为不变（成功完成后不关闭 Edge，出错时关闭 Edge）。
