...
| 模板 | 方法 | 参数 |
|------|------|------|
| `白W3.psd` | legacy | `scale=0.40`, `rotation_degrees=0`, `effective_top_y=450`, `effective_center_x=730`, `blend_mode=multiply` |
| `白B1.png` | transform | `scale=0.40`, `rotation_degrees=0`, `effective_top_y=725`, `effective_center_x=649`, `blend_mode=multiply` |
| `白B3.png` | transform | `scale=0.32`, `rotation_degrees=-3.0`（逆时针 3°）, `effective_top_y=700`, `effective_center_x=777`, `blend_mode=multiply` |
| `白B4.png` | transform | `scale=0.28`, `rotation_degrees=2.0`（顺时针 2°）, `effective_top_y=1011`, `effective_center_x=576`, `blend_mode=multiply` |
| `白正2.jpg` | transform | `scale=0.20`, `rotation_degrees=-2.0`（逆时针 2°）, `effective_top_y=644`, `effective_center_x=670`, `blend_mode=multiply` |

新增胚衣时，先让用户用 Excel 填写 [`胚衣参数表_模板.csv`](./胚衣参数表_模板.csv) 并保存/导出为 CSV UTF-8；再读取该 CSV，转换成 [`胚衣参数表.md`](./胚衣参数表.md) 并新增一条 preset；不要把已有模板的参数硬编码进代码或测试。
