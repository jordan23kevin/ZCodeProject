...
## v1.8.4 — 2026-08-21（成品体积目标下调到 120~160KB，用户实测选定）

- `_save_jpeg_range` 默认区间 200~300KB → **120~160KB**，质量下限 60→45 保底。
- 用户对比实测选定：q45(<100KB) 印花高对比边缘有可见毛边不建议；120~160KB≈q63~70 边缘干净、体积约为原 q95 的 1/3。
- 实测：明黄平铺 131KB@q63、黑模特 143KB@q70、白平铺 133KB@q63。

## v1.8.3 — 2026-08-21（成品 JPEG 体积自适应 200~300KB）

- 用户需求：去背预览页点贴图生成的成品图控制在 200~300KB，清晰度尽量少损。
- `core.py` 新增 `_save_jpeg_range`：从 max_quality（默认 95）起二分降质直到体积 ≤300KB，
  落在 [200,300] 即停；低于 200KB 不强行升质（质量上限 max_quality），降到 q60 仍超则接受最小体积。
  Pillow 在 quality<95 时自动从 4:4:4 切 4:2:0 色度抽样，二分自然利用这一跳变。
- 4 处成品保存点全部改走该函数（T恤/卫衣、平铺/模特全生效）。
- 实测（1340×1785 成品二次编码）：480~490KB → 200~237KB @q77~81，印花边缘锐利无可见色块。

## v1.8.2 — 2026-08-19（manual 遮罩探测兼容英文色带空格胚衣名）

- `core.py`：manual 自动探测先试 `<胚衣名>_manual.png`，找不到再试 `<胚衣名去空格>_manual.png`——英文色文件夹胚衣 ` B2.jpg`（带前导空格）用户手动保存的遮罩通常是不带空格的 `B2_manual.png`，此前探测 ` B2_manual.png` 永远找不到、遮罩不生效。
- 同步 ps 链 v2.6.0 同一修复。
