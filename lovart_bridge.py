...
def _load_upload_date_map():
    """解析 D:\Semems WB\已上款货号_wb.md，返回 {DXxxxx: 'yymmdd'}。
    格式：
        ## 2026-06-27
        * DX0001
        * DX0002
    同一 DX 多次出现时，以后出现的日期为准（可覆盖重新上款）。
    """
    date_map = {}
    if not UPLOAD_RECORD_MD.exists():
        return date_map
    try:
        text = UPLOAD_RECORD_MD.read_text(encoding="utf-8")
    except Exception:
        return date_map
    current_date = ""
    date_re = re.compile(r"^##\s*(\d{4})-(\d{2})-(\d{2})\s*$")
    dx_re = re.compile(r"^[-*]\s*(DX\d+)\s*$")
    for line in text.splitlines():
        line = line.strip()
        m = date_re.match(line)
        if m:
            current_date = f"{m.group(1)[2:]}{m.group(2)}{m.group(3)}"  # 2026-06-27 -> 260627
            continue
        m = dx_re.match(line)
        if m and current_date:
            date_map[m.group(1)] = current_date
    return date_map


def _scan_upload_projects():
    """扫描所有 DX 的 03_UPLOAD，返回 [{dx, date, files:[{name,mtime}]}]
    date 优先从 D:\Semems WB\已上款货号_wb.md 读取；
    未记录的款回退到 AI 生成图最新 mtime（无 AI 时退去背图 mtime），与 check_rem.py 保持一致。
    """
    projects = []
    if not PROJECTS_DIR.exists():
        return projects
    upload_date_map = _load_upload_date_map()
    for d in sorted(PROJECTS_DIR.iterdir()):
        ...
        if not files:
            continue
        dx = d.name
        # date 优先级：已上款记录 > AI mtime > rembg mtime
        dx_date = upload_date_map.get(dx, "")
        if not dx_date:
            ai_dir = d / "01_AI"
            rem_dir = d / "02_REM_BG"
            date_paths = []
