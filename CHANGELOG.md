...
## v2.1.9 (2026-07-02) — 修复进度百分比超标

### 🐛 修复

- **进度条显示异常**：`268 / 13 (2062%)`
  - 根因：`/api/upload/progress` 把历史 `已上款货号_wb.md` 中的 268 条记录全部算进 done_count
  - 解决：`done_count` 只统计「本次选中的款」里已完成的数量，`fail_count` 同理
  - 页面分区仍使用历史已完成记录，进度条不再被历史记录撑爆

---

## v2.1.8 (2026-07-02) — 精确上款 + 进度条 + 已上款分区

### 📤 上款精确控制

- **`/api/batch-upload` 改为 `--only` 模式**：勾选哪款就上哪款，不再以最早 DX 为起点连续处理后续款
- **解决「勾选 DX0255 却上 DX0263」**：原逻辑把第一个 DX 当起点，脚本会自动往后找未上款；新逻辑精确传入选中款号

### 📊 上款页面升级

- **页面拆分为「未上款 / 已上款」两个区域**：已上款自动沉底，未上款置顶
- **新增进度条面板**：
  - 当前正在上款的 DX
  - 进度百分比（完成数 / 总数）
  - 已用时间、平均每款耗时、预计剩余时间
- **勾选逻辑调整**：「全选」只选未上款卡片，避免重复勾选已上款
- **修复悬停放大图位置**：缩略图在屏幕下方时，放大图自动显示在缩略图上方，避免被裁切

### 🔧 后端 API

- 新增 `GET /api/upload/progress` — 读取 `D:\Semems WB\.wb_upload_progress.json`
- `upload.html` 每 2 秒轮询进度，完成后自动停止并刷新分区

### 🐛 店小秘图片残留修复

- `wb_listing.py v1.3.0` 上传图片前会**删除每行前2张旧图**（先删第一张，DOM 更新后再删第二张）
- 根因：店小秘「引用产品」会保留上一款图片，原代码只删前两张，导致旧图残留
- 解决：进入图片表格后，按颜色行逐行清空所有已有图片，再上传新图片

### 🏗️ 依赖项目更新

- `E:\Claude code\wb上款\wb_listing.py` → v1.3.0
  - 支持 `--only DXxxxx --only DXyyyy ...` 多选精确上款
  - 新增进度追踪 JSON：`D:\Semems WB\.wb_upload_progress.json`
  - 上传前删除每行前2张旧图
