...
### Fixed
- **位移扭曲"水波纹"**：`apply_displacement()` 直接用原始 disp 图逐像素偏移，disp 中的小褶皱/布纹高频分量导致印花（尤其文字）像水波纹一样抖动。新增两位移控制：
  - `--disp-smooth`（默认 80）：对 disp 图做大尺度高斯平滑后再取偏移，**只保留大褶皱形变**，抹掉小褶皱高频。
  - `--disp-dead-zone`（默认 15 灰度级）：`|disp-128|` 小于死区的区域偏移强制为 0，**小起伏区域完全不扭曲**。
  - 传 0 可恢复旧行为。`apply_mockup` / `apply_mockup_transform` 均支持。

### Changed
- `w_mockup_extra.py`（check_rem 自动贴图链路）`--disp-strength` **30 → 18**：配合位移低频化，大褶皱自然弯曲但不过度扭曲（旧值 30 是水波纹主因之一）。
- 实测（DX0635 → 黑W11 tpl 端到端）：旧（s30/不平滑）文字波浪抖动；新（s18/smooth80/dz15）文字平稳、仅随大褶皱轻微弯曲。
