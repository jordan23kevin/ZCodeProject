...
## engine/check_rem v2.7.5 (2026-08-21) — T恤审图改"同色同一行" + 网格缩略图按单元尺寸提清晰度

- **T恤审图行模式**：T恤只有黑/白两色，列模式只剩 2 列、横向空间浪费、图小看不清。现按品类分流（模板注入 `var CAT`）：`wb` → 同色**同一行**（色标在行首，行内横排铺满）；`hoodie` 等多色品类保持同色同列不变。分带算法行/列两态各自做最优适配。
- **网格清晰度**：`/thumb` 新增 `?sz=` 边长参数（默认 300、封顶 1200），审图网格按单元实际宽度请求对应尺寸（`ceil(cellW/100)*100`），大图不再糊；非默认尺寸单独存缓存（`...{srcsize}.{size}px.jpg`），300px 默认缓存不受影响、不重建。
- 已实开浏览器验证：T恤 DX3543BW（6 张/2 色）→ 黑行+白行两行、单元 332×414、sz=400、同色同 y 坐标；卫衣 HX0004BW 回归——仍 10 列×4 行、141×175、sz=300 不变；均无 JS 报错。
- 已同步两个生产副本字节一致并重启 8766/8767 实例生效。

## engine/check_rem v2.7.4 (2026-08-21) — 修复缩略图撞图（浅黄/明黄同图）+ 卡片页整齐化重设计

- **缩略图撞图修复**：根因——贴图管线覆盖成品文件时保留旧 mtime（copy2/move），`get_thumb` 只靠 mtime 判新鲜，把覆盖前生成的旧内容缩略图当新鲜返回（实测 HX0004BW 明黄/浅黄两张缩略图字节完全相同）。缓存键改为 **文件名+源文件大小**（`{dx}__{kind}__{file}.{size}.jpg`），内容替换后大小变化即自动重生成；旧格式缓存已全部清扫（WB 16476 个 / 371MB，Hoodie 1530 个 / 25MB，新缓存按需懒生成）。已实测两色缩略图内容不同。
- **卡片页整齐化重设计**（功能不变，适配"1 张贴图 / 2 张 / 1 张+反黑反白"各种款型）：
  - 卡片定宽 **480px**（原 auto-fill minmax(520px,1fr) 拉伸不齐），网格居中、顶部对齐，1600 宽屏 3 列；
  - 每个设计一条横带：badge + AI + REM 缩略图固定 150×150，B/W/BW 竖向叠放（原左右半栏宽度不一），缺图用同尺寸占位块；
  - 反黑/反白变体收成紧凑横条（88px 小图一排），不再占整行大格；
  - 单元格按钮缩小一号（12px/20px 行高），150px 宽下放得下。
- 已实开浏览器验证：卫衣 40 款 / T恤 30 款卡片宽度全部=480、零横向溢出；审图总览 10 色列完整、明黄/浅黄显示正确；无 JS 报错。
- 已同步两个生产副本字节一致并重启 8766/8767 实例生效。

## engine/check_rem v2.7.3 (2026-08-21) — 修复审图总览左右边缘图被裁切

- 根因：`renderRevGrid` 可用宽按 `innerWidth-60` 算，比 `.rev-main` 实际内容盒（70px×2 内边距）宽 80px，网格溢出到两侧内边距区，被 52px 宽的翻页箭头压住。
- 翻页箭头缩小：52×84/40px → **34×56/26px**，位置 left/right 10px→6px，加 z-index:6；`.rev-main` 内边距 70px→48px（腾出空间）；单图模式大图上限 100vw-160px→100vw-120px。
- 网格可用区改为**实测 `.rev-main` 的 clientWidth/clientHeight**（-104/-28 余量），不再靠猜常量；容器不可测时回退窗口尺寸。
- 顺带修复顺序 bug：`openRev` 原来先 `renderRev()` 再加 `active`（overlay 还 display:none，clientWidth=0 → 网格塌成 80px 兜底），现先显示再渲染。
- 已实开浏览器验证（8767 卫衣，1600×950）：HX0004BW 40 张 10 列×4 行 141×175、minL=43/maxR=1543 不碰箭头（箭头占 6~40）；HX0005B 20 张 5 列×4 行 155×193、上下左右全在视口内；均无 JS 报错。
- 已同步两个生产副本字节一致并重启 8766/8767 实例生效（CSS 在 .py 模板内需重启；JS 磁盘现读）。

## engine/check_rem v2.7.2 (2026-08-21) — 已贴图只留审图入口 + 总览网格满幅自适应（自动分带）

- 卡片「已贴图」区不再平铺缩略图（外层不再显示成品图），只留 `📎 已贴图 N 张 [🔍 审图]` 入口，点审图才展示；`copyNoSticker` 依赖的"未贴图"空 bar 判定保持不变。
- 审图总览网格重做（用户规则：图片尽量大、一屏看清整款）：
  - 仍**同色同列**（列=色，列头带色名），单元按 4/5 比例**宽高分开算**——宽=min(列宽约束, 高约束/1.25)、高=宽×1.25，图片填满不留白边；
  - **自动分带**：色多图少时从 1..色数遍历带数取单元最大者，颜色按最优列数切成多带上下排列（同色仍同列），20 张款不再被 40 张款的小单元拖小；
  - 视口余量收紧（`availW=innerWidth-padX-20`、`availH=innerHeight-topH-bottomPad-12`），修复 40 张网格左右溢出被裁切的问题。
- 单图模式不变（大图+↻重贴+×删除+胶片条+←/→跨款），Esc 两级回退（单图→总览→关闭），总览 ←/→ 整款切换，顶栏「⊞ 总览」仅单图模式显示。
- 已实开浏览器验证（8767 卫衣）：HX0004BW 40 张=10 列×4 行、单元 147×183 满幅无溢出（minL=13, maxR=1573 ≤ 视口 1600）；HX0005B 20 张自动分 2 带（5 列×4 行×2）、单元 156×195；单图/Esc 回退/跨款翻页回归通过，无 JS 报错。
- 设计取舍说明：40 张=10 色×4 张，"同色同列"硬约束下几何上只能 10 列×4 行（无法 4 列×10 行）；20 张在同约束下物理上限≈156×195（两带 5 列×4 行），已接近上限。
- 已同步 `D:\Semems WB\04_OS\engine\` 与 `D:\Semems Hoodie\04_OS\engine\` 字节一致；check_rem.js 为静态文件磁盘现读，无需再次重启实例。

## engine/check_rem v2.7.1 (2026-08-21) — 已贴图固定两列 + 审图总览网格（同色同列）

- 已贴图区统一两列（原 T恤 BW 款 3 列规则取消，用户规则：所有款两列）。
- 审图改双层模式：
  - **总览网格（进入默认）**：一款全部成品一屏铺开，**同一颜色固定在同一列**（列=色，列头带色名），行列与单元尺寸按视口自动计算（40 张=10 色×4 张 → 10 列×4 行；20 张同理自适应）；每张带文件名角标，悬停出 × 可删（删完留总览）；←/→ 整款切换。
  - **单图模式**：点任意图进入——大图 + ↻重贴（左上）+ ×删除（右上）+ 胶片条 + ←/→ 翻页跨款；顶栏「⊞ 总览」或 Esc 回总览，再 Esc 关闭。
- 数据结构：UP_FILES 每项新增 `c`（衫色，由 _upload_detail 分组时带出）。
- 已实开浏览器验证（8767 卫衣 HX0004BW 40 张）：总览 10 列×4 张/40 cells/角标齐、单图 6/40、Esc 两级回退、总览 ←/→ 换款（HX0004BW→HX0005B 20 张）、删除/重贴按钮就位；已贴图区两列确认；无 JS 报错。
- 已同步 `D:\Semems WB\04_OS\engine\` 与 `D:\Semems Hoodie\04_OS\engine\` 字节一致，8766/8767 实例已重启。

## engine/check_rem v2.7.0 (2026-08-21) — AI 去背 贴图 OS 界面降噪 + 成品全屏审图

- 工具栏分两层：第 1 行筛选（日期/搜索/刷新全部/PS状态/计数），第 2 行批量操作（全选/批量去背/批量贴图/批量反黑/批量反白/复制缺图款号/复制缺贴图），功能与 id/事件全部原样保留。
- 降噪：格子按钮栏里的长文件名（`.btn-stem`）隐藏显示（悬停 title 仍可见）。
- 成品全屏审图（解决"贴图完成后审图不好看"）：成品缩略图点击或「已贴图」行头 🔍 审图进入——大图居中、←/→/Esc、底部胶片条、跨款连续翻页（末张→下一款首张，反向同理，跳过无成品款）；删除 × 放右上角、重新贴图 ↻ 放左上角、左下角文件名角标，删除/重贴复用 /del、/resticker 接口并同步卡片状态与破缓存。
- 数据源：模板注入 `UP_FILES`（{dx:[{n,t,l}]}，由 _upload_detail 填充）与 `DX_ORDER`；悬浮放大预览保持不变。
- 已实开浏览器验证（8766 T恤）：工具栏 2 行、8 个功能按钮全在、审图开/翻页/跨款 DX3541BW→DX3542W/回退、Esc 关闭、缩略图点击进审图、悬浮预览，均无报错；8767 卫衣实例同步重启冒烟通过。
- 已按协议硬规则②同步副本：`D:\Semems WB\04_OS\engine\` 与 `D:\Semems Hoodie\04_OS\engine\` 均与 ZCodeProject 字节一致，两个常驻实例（8766/8767）已重启生效。

## upload.html v2.5.4 (2026-08-21) — 展开图流全宽两行大图（卡片原地不动）

- 修正 v2.5.3 的取舍错误：v2.5.3 为了不占独立行把图流缩进卡内（140px 太小）；现改为**卡片在原格子不动，展开时图流节点搬到卡片正后方的全宽带**（`.gal-band{grid-column:1/-1}`，`insertBefore(card.nextSibling)`），两行横向大图流占满屏幕宽度，缩略图回到 230px（4/5≈288px 高），不悬浮也能看清细节；悬浮放大预览保留。
- 展开状态持久化：新增 `EXPANDED` 集合，renderCards 轮询重建 DOM 后自动恢复展开（`expandGallery` 重搬节点），收起时图流搬回卡内并移除全宽带。
- 已实开浏览器验证：展开前后卡片坐标不变（top=539）、全宽带 1541px、rows=2、40 项、thumb 230×288、renderCards 重建后展开自动恢复、收起完全复位；无 JS 报错。

## upload.html v2.5.3 + lovart_bridge v2.6.4 (2026-08-21) — 原位展开 / 审图一屏3张 / 📂修复

- 展开回原位：去掉 v2.5.1 的"展开卡片占满整行"（用户反馈卡片掉到独立一行不好），现在卡片在原位置就地展开，卡内两行横向图流（缩略图 140px，悬浮放大预览保留）。
- 审图模式一屏 3 张：主区改 3 格网格，左右箭头/键盘一次翻 3 张；跨款规则顺延（右过界→下一款第 1 组，左过界→上一款最后一组）；删除按钮从顶栏移到每张图右上角（×），每张图左下角带文件名角标；顶栏序号显示区间（如 4-6 / 40）。
- 修复 📂 打不开文件夹（bridge v2.6.4）：/api/open 原来写死 DX 前缀 + 固定 D:\Semems WB，卫衣 HX 直接 400；现按 ?cat= 走品类上下文（_upload_cat_guard + _dx_re(ctx["prefix"])），前端 openDxFolder 补 withCat 并加失败 toast。
- 已实开浏览器验证：审图 1-3/40 → →翻 4-6 → 末组 40-40 → 跨款 1-3/20 → ←回退；展开卡宽不变、两行 40 项；/api/open 对 DX/HX 均 ok:true；无 JS 报错。

## upload.html v2.5.2 (2026-08-21) — 审图模式跨款连续翻页

- 键盘 ←/→ 或左右箭头翻页越过本款边界时自动跨款：最后一张再按 → 进入下一款第一张；第一张再按 ← 回到上一款最后一张（`revJump` 沿 `DX_ORDER` 找下一个有未删图的款，自动跳过无图款）。
- 到全局尽头（第一款第一张 / 最后一款最后一张）不再回绕，toast 提示"已经是第一张（第一款）/ 最后一张（最后一款）"。
- 新增 `DX_ORDER`（renderCards 时按 未上款+已上款 展示顺序重建），替代不可靠的 DX_FILES 键序。
- 已实开浏览器验证：40 张款末位 → 跨到下一款 1/20、反向回到上一款 40/40、全局起点 toast、款内正常翻页，均无报错。

## upload.html v2.5.1 (2026-08-21) — 封面颜色轮换 + 展开改为全宽两行大图流

- 封面轮换：原来每款封面都取第一张图（整屏同色），现按卡片序号在颜色组间轮换（`_renderSeq % 组数`），整屏颜色错开一眼可辨；封面点击进审图时定位到封面那张图（不再是固定的第 0 张）。
- 展开区重做：展开的卡片占满整行（`.card.expanded{grid-column:1/-1}`，封面收为 320px 居中），图片区改为「两行 × 横向滚动」grid 流（`grid-auto-flow:column` + `grid-template-rows:repeat(2,auto)`），充分利用横向空间，40 张图两行排完、左右滚动浏览。
- 缩略图加大到 230px 宽（4/5 比例，约 288px 高），接近封面尺寸，不悬浮也能看清细节；每张左上角带颜色徽章；悬浮放大预览（bindPreview）与点击进审图均保留。
- 已实开浏览器验证：封面 8 张卡 8 种颜色、两行流 rows=2 / 40 项 / 横向 scrollWidth 4870px、悬浮预览、审图定位、收起复位全部正常，无 JS 报错。
...
- 新增 `_kill_stale_wb_listing`：`/api/batch-upload` 启动前用 PowerShell 找出命令行含 `wb_listing.py` 的 python 进程并 `taskkill /F`（不误杀其他进程），杀完等 0.5s 释放文件句柄；清理动作放在 force 删记录**之前**，从源头消除"僵尸进程占用 md 文件 → 删记录 Permission denied → 静默空跑"（v2.6.2 只是报错拦截，本版直接治本）。
- 返回体新增 `stale_killed` 字段（被杀 PID 列表），前端可按需展示。
...
- T恤 成品展示行按款型分列：BW 款每色 3 张（B/W 平铺+BW合成）→ 3 列；单 W/单 B 每色 2 张 → 2 列；
  卫衣保持固定 2 列。已实测：wb 页 20 行 3 列 + 4 行 2 列，hoodie 页 321 行全部 2 列。
