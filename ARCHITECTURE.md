...
**解决方案**：`handle_colors_images()` 进入图片表格后，调用 `_clear_old_images()` 每行最多点击 10 次 `.icon_delete`，
确保引用产品和上一款图片全部清空，然后再上传当前 SKU 的 `03_UPLOAD` 图片。

### 上款速度优化（v1.3.1）

- `wb_listing.py` 每颜色批量上传 3 张图，再拖拽排序为 BW,B,W；失败自动回退单张上传
- `_clear_old_images()` 彻底清空旧图，避免 DX0282 的 W 图残留在 DX0273 页面
