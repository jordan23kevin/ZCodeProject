...
**根因**：原代码只删除每行「前两张图」，但引用产品可能遗留第三张旧图；且只删一次，DOM 更新后未继续清理。

**解决方案**：`handle_colors_images()` 进入图片表格后，先删每行第一张图，等 DOM 更新后再分别删除第二、第三张图，
确保每行留出 3 个空位给当前 SKU 的 `03_UPLOAD` 图片，然后再上传。
